...
from gpt2_models import GPT2LMSimClsHeadModel
...
def compute_metrics(outputs):
    # 从 outputs 中提取 loss 和 label_loss
    loss = outputs.loss
    label_loss = outputs.label_loss
    # 返回一个字典，包含 loss 和 label_loss
    return {"loss": loss, "label_loss": label_loss}
...
def main():
    ...
    parser.add_argument("--pretrain_data_path", type=str,
                        default=data_root + "/5minData_bin9/preprocessed_data/5min.txt")
    parser.add_argument("--validation_data_path", type=str,
                        default=data_root + "/5minData_Val_bin9/preprocessed_data/5min_val.txt")
    parser.add_argument("--pretrain_cls_labels_path", type=str,
                        default=data_root + "/5minData_bin9/preprocessed_data/cls_labels.json")
    parser.add_argument("--pretrain_token_vecs_dict_path", type=str,
                        default=data_root + "/5minData_bin9/preprocessed_data/token_vecs_dict.json")
    parser.add_argument("--pretrain_model_path", type=str, default=data_root + "/ckpt/===")
    parser.add_argument("--data_caches", type=str, default=data_root + "/caches_bin9_5min_sim_cls")
    parser.add_argument("--config_path", type=str,
                        default=data_root + "/gpt_vocab_bin9/config.json")
    parser.add_argument("--tokenizer_path", type=str,
                        default=data_root + "/gpt2_tokenizer_bin9")
    parser.add_argument("--checkpoint_save_path", type=str,
                        default=data_root + "/checkpoint_kbar_bin9_5min_CLS_SIM")
    parser.add_argument("--save_path", type=str, default=data_root + "/5min-gpt-base-kbar-bin9-CLS-SIM")
    ...
    parser.add_argument("--save_total_limit", type=int, default=15)  # 限制checkpoints的数量，最多15个
    ...
    word_weights_dict = {word: (total_words / count) ** 0.5 for word, count in word_counts.items()}
    ...
    # print(cls_labels.shape, len(vocab))
    # cls_labels_embed = nn.Embedding(len(vocab), 1, _weight=cls_labels)
    ...
    validation_dataset = KbarTextDataset(tokenizer=tokenizer, block_size=args.max_seq_len, file_path=args.validation_data_path)
    validation_dataset.examples = validation_dataset.examples[:int(len(validation_dataset.examples) / 2)]
