                        ...
                        for i in range(0, len(tokenized_text) - block_size + 1, block_size):  # Truncate in block of block_size
                            try:
                                self.examples.append(
                                    torch.tensor(tokenizer.build_inputs_with_special_tokens(tokenized_text[i: i + block_size]))
                                )
                            except ValueError:
                                print(text.split(' ')[i: i + block_size])
                                break
                    else:
                        self.examples.append(
                            torch.tensor(tokenizer.build_inputs_with_special_tokens(tokenized_text))
                        )
                self.examples = pad_sequence(self.examples, batch_first=True, padding_value=padding_token_id)
