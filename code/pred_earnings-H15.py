...
parser.add_argument("--save_path", type=str,
                    default="/INPUT/lwb/k-emebedding/NORMAL/H-15_lseq-gpt-base-kbar-bin13-SIM")
    ...
    if os.path.isfile(csv_file_val):
        ...
        while current_time <= end_time:
            ...
            if frame_15min.empty:
                ...
                opens = list(frame_15min["open_15min"])
                closes = list(frame_15min["close_15min"])
                highs = list(frame_15min["high_15min"])
                lows = list(frame_15min["low_15min"])
...
if __name__ == "__main__":
    ...
    saved_names = list(vols_dict_H15.keys())
    ...
    for item_name in item_names:
        ...
        plt.savefig(png_save_path + item_name + "Cumulative Returns-H15.png", format='png', dpi=300)
        plt.close()
