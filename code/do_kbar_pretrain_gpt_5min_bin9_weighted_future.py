...
from gpt2_models import GPT2LMWeightedHeadModel
...
def main():
    data_root = "/data/users/lwb/k-emebedding/NORMAL"
    ...
    parser.add_argument("--data_caches", type=str, default=data_root + "/caches_bin9_5min")
    ...
    parser.add_argument("--checkpoint_save_path", type=str,
                        default=data_root + "/checkpoint_kbar_bin9_5min")
    parser.add_argument("--save_path", type=str, default=data_root + "/5min-gpt-base-kbar-bin9")
    ...
    # word_weights = [word_weights_dict[word] if word in word_weights_dict else 0.0001 for word in vocabulary]
    # 定义词汇表
    # 定义词汇表
    ...
    print("sentence size:", len(training_corpus))

    gpt2_config = GPT2Config.from_pretrained(args.config_path)
    model = GPT2LMWeightedHeadModel(config=gpt2_config, weight=word_weights)
